...
describe('Platform', () => {
    ...
    const sampleUserAgents: { [key: string]: string[] } = {
        'chrome': [
    ...
    const testIsFn = (identifier: Identifier, ua: string, expectation = true) => {
    ...
    const runTestsForBrowser = (browserName: string, identifier: Identifier) => {
        // Test valid user-agents
        sampleUserAgents[browserName].forEach((ua: string) => testIsFn(identifier, ua));

        // Test invalid user-agents
        Object.keys(sampleUserAgents)
                .filter((browser: string) => browser !== browserName)
                .map((browser: string) => sampleUserAgents[browser])
                .forEach((invalidBrowserUserAgents: string[]) => invalidBrowserUserAgents.forEach((ua: string) => testIsFn(identifier, ua, false)));
    };

    describe('Chrome', () => {
        runTestsForBrowser('chrome', Browser.CHROME);
    ...
    describe('Firefox', () => {
        runTestsForBrowser('firefox', Browser.FIREFOX);
    });

    describe('Internet Explorer', () => {
        runTestsForBrowser('ie', Browser.IE);
    });

    describe('Edge', () => {
        runTestsForBrowser('edge', Browser.EDGE);
    });

    describe('Opera', () => {
        runTestsForBrowser('opera', Browser.OPERA);
    });

    describe('Safari', () => {
        runTestsForBrowser('safari', Browser.SAFARI);
    });
